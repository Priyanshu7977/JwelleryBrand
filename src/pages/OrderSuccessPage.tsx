    ...
    if (!order && orderId) {
      ...
      getOrderById(orderId)
        .then((fetched) => {
          setOrder(fetched);
          if (fetched) {
            // Trigger automatic email confirmation in background
    ...
    <div className="w-full min-h-screen bg-pearl-100 pt-28 sm:pt-32 pb-24 px-4 sm:px-6 md:px-10 lg:px-14 selection:bg-champagne-300">
      <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-champagne-300/80 shadow-luxury-soft p-6 sm:p-8 space-y-6 text-center">
          ...
          {/* Real Automated Email Status Bar */}
          <div className="p-4 rounded-2xl bg-sand/40 border border-champagne-300/70 max-w-2xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-left">
              ...
              <div>
                <p className="text-xs font-bold text-obsidian flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-mono bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded font-bold">
                    {emailStatus === 'resending' ? 'Sending...' : 'Delivered'}
                  </span>
                </p>
                <p className="text-[11px] text-obsidian-soft truncate max-w-[280px] sm:max-w-md">
                  Transmitted to <strong className="text-obsidian">{order.customer.email}</strong> & Atelier Desk
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
              <button
                ...
                className="flex-1 sm:flex-initial px-3.5 py-1.5 rounded-full bg-white hover:bg-champagne-100 text-obsidian border border-champagne-300/80 text-[11px] font-bold transition-all flex items-center justify-center gap-1.5 shadow-xs"
              >
                <RefreshCw className={`w-3 h-3 ${emailStatus === 'resending' ? 'animate-spin' : ''}`} />
                <span>{emailStatus === 'resent' ? 'Sent Again!' : 'Resend Email'}</span>
              </button>
              <button
                onClick={() => window.print()}
                className="p-1.5 rounded-full border border-champagne-300/80 hover:bg-champagne-100 text-obsidian transition-colors"
