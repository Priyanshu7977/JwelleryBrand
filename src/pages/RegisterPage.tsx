...
export const RegisterPage: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(false);
    if (res.success) {
      showToast(`Welcome to the Celestia Family, ${formData.name}! ✨`);
      navigate('/account');
    } else {
  ...
  return (
    <div className="w-full min-h-screen bg-pearl-100 pt-24 sm:pt-32 pb-20 sm:pb-32 px-4 sm:px-6 flex items-center justify-center">
      <SEOHead
        title="Create an Account | CELESTIA Patron Family"
        description="Join Celestia to save customized gift hampers, earn loyalty perks, and track orders seamlessly."
        canonical="https://jwellery-brand.vercel.app/register"
        noIndex={true}
      />
